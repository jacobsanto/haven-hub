...
export function PropertyStickyNav({ sections, propertyName }: PropertyStickyNavProps) {
  ...
  useEffect(() => {
    const handleScroll = () => {
      // Show nav after scrolling past 400px (hero area)
        ...
        >
          <div className="container mx-auto px-4">
            <div className="flex items-center h-14 md:h-16 gap-4 overflow-x-auto scrollbar-hide">
              {/* Property name - visible on larger screens */}
              ...
              <nav className="flex items-center gap-1 md:gap-2">
                {sections.map((section) => (
                  <button
                    ...
                    className={cn(
                      'px-3 py-1.5 md:px-4 md:py-2 text-sm font-medium rounded-full transition-all duration-200 whitespace-nowrap',
