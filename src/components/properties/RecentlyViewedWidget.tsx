...
export function RecentlyViewedWidget({ 
  ...
  // Filter out current property if on detail page
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs font-semibold text-primary">
                    From ${item.base_price.toLocaleString()}/night
                  </p>
                </div>
              </Link>
            ))}
  ...
  // Inline variant (for Properties page, etc.)
  return (
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        {filteredItems.map((item, index) => (
          ...
          >
            <Link
              to={`/properties/${item.slug}`}
              className="group block bg-card rounded-xl overflow-hidden border border-border hover:border-primary/30 hover:shadow-lg transition-all"
            >
              {/* Image */}
